...
def average_close(df, index, ticker, fig):
    ...
    if data is not None:
        ...
        trace = go.Scattergl(
            x = df.index,
            y = (np.zeros(len(df.index)) + ftwh),
            mode = 'lines', name = '52wh',
            showlegend = False,
            line = {'color': 'rgb(255, 255, 255)', 'width': 1, 'dash': 'dot'}
            )
        fig.append_trace(trace, 1, 1)
        trace = go.Scattergl(
            x = df.index,
            y = (np.zeros(len(df.index)) + df['Close'].mean()),
